...
<header class="header js-header">
    <div class="header__container container">
        ...
        <a href="<?php echo $home; ?>" class="header__logo">
            <img class="header__logo-img" src="<?php echo $logo; ?>" title="<?php echo $name; ?>" alt="<?php echo $name; ?>">
        </a>
        <?php } ?>
            ...
            <div class="header__phones">
                <?php if($telephones){ ?>
                    <?php foreach($telephones as $telephone){ ?>
                        <a href="tel:<?=$telephone;?>" class="link"><?=$telephone;?></a>
                    <?php } ?>
                <?php } ?>
            </div>
            <button class="button js-show-contact-popup">Консультация</button>
        </div>
    </div>
</header>
<div class="header__placeholder js-header-placeholder"></div>
		...
		<li class="col<?php echo $category_1['column']; ?> current">
         ...
         <?php } ?>
          ...
          <ul>
                ...
                <?php } ?>
              <?php if ($category_2['children'] && false) { ?>
                <div class="third">
                  <ul>
                   <?php foreach ($category_2['children'] as $category_3) { ?>
                   <li><a href="<?php echo $category_3['href']; ?>"><?php echo $category_3['name']; ?></a></li>
...
<?php if ($categories && $class == 'common-home') { ?>    
<div class="category-nav">
    <div class="category-nav__container container">
        <?php foreach ($categories as $key=>$category_1) { ?>
        
        <div class="category-nav__item _c<?php echo $key+1; ?>">
            <div class="category-nav__item-wrap">
                <a href="<?php echo $category_1['href']; ?>">
                <div class="category-nav__item-icon"></div>
                </a>
                <a href="<?php echo $category_1['href']; ?>">
                <span class="category-nav__item-title">
                    <?php echo $category_1['name']; ?>
                </span>
                </a>
                <?php if ($category_1['children']) { ?>
                <div class="category-nav__menu">
                    <div class="category-nav__menu-arrow"></div>
                    <div class="category-nav__menu-container">
                        <?php foreach ($category_1['children'] as $category_2) { ?>
                        <div class="category-nav__menu-item">
                            <a href="<?php echo $category_2['href']; ?>" class="category-nav__menu-item-wrap">
                                <?php if($category_2['thumb']){ ?>
                                    <img src="<?php echo $category_2['thumb']; ?>" alt="<?php echo $category_2['name']; ?>" class="category-nav__menu-item-img">
                                <?php } ?>
                                <button class="button">Подробнее</button>
                            </a>
                            <span class="category-nav__menu-item-title"><?php echo $category_2['name']; ?></span>
                        </div>
                        <?php } ?>
                    </div>
                </div>
        ...
        <style>
        .category-nav__item._c<?php echo $key + 1; ?> .category-nav__item-icon {
            background-image: url("<?php echo $category_1['thumb']; ?>");
        }
        .category-nav__item._c<?php echo $key + 1; ?> .category-nav__item-wrap:hover .category-nav__item-icon {
            background-image: url("<?php echo $category_1['thumb2']; ?>");
        }
        </style>
        <?php } ?>
        <?php } ?>
        
        <?php foreach ($blog as $key2=>$category_1) { ?>
        
        <div class="category-nav__item _c<?php echo $key + $key2 + 2; ?>">
            <div class="category-nav__item-wrap">
                <a href="<?php echo $category_1['href']; ?>">
                <div class="category-nav__item-icon"></div>
                </a>
        ...
        <?php if($category_1['thumb']){ ?>
        <style>
        .category-nav__item._c<?php echo $key + $key2 + 2; ?> .category-nav__item-icon {
            background-image: url("<?php echo $category_1['thumb']; ?>");
        }
        .category-nav__item._c<?php echo $key + $key2 + 2; ?> .category-nav__item-wrap:hover .category-nav__item-icon {
            background-image: url("<?php echo $category_1['thumb2']; ?>");
        }
        </style>
        <?php } ?>
...
<div class="container">
    <nav class="nav">
        <div class="nav__container">
            <?php for($i=1; $i<=6; $i++){ ?>
                <?php if(${'cosyone_custom_menu_title'.$i}){ ?>
                    <a class="nav__link <?=(${'cosyone_custom_menu_url'.$i} == $request_url)? '_active' : '' ?>" href="<?php echo ${'cosyone_custom_menu_url'.$i}; ?>"><?php echo ${'cosyone_custom_menu_title'.$i}; ?></a>
                <?php } ?>
            <?php } ?>
        </div>
    </nav>
