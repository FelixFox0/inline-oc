...
<?php } ?>
    ...
    <div class="popup__card-images">
        <div class="popup__card-showcase">
            <?php if ($thumb) { ?>
                <img class="js-item-showcase-img" src="<?php echo $popup; ?>">
            <?php } ?>
        </div>
        <div class="popup__card-images-row">
            <div class="popup__card-image">
                <img class="js-item-thumb-img" src="<?php echo $popup; ?>">
            </div>
            <?php foreach ($images as $image) { ?>
                <div class="popup__card-image">
                    <img class="js-item-thumb-img" src="<?php echo $image['popup']; ?>">
                </div>
            <?php } ?>
        </div>
    </div>
    <div id="product" class="popup__card-data">
        <div class="popup__card-data-title">
            <?php echo $heading_title; ?>
        </div>
        <div class="popup__card-data-model">
            Модель: <?php echo $model; ?>
        </div>
        <div class="popup__card-data-desc"><?php echo $description; ?></div>
        
       <input type="hidden" name="product_id" value="<?php echo $product_id; ?>" />
       <input type="hidden" name="quantity" value="<?php echo $minimum; ?>" size="2" id="input-quantity" class="quantity" />
        <ul class="popup__card-list">
            <?php foreach($attribute_groups as $group){ ?>
                <?php foreach($group['attribute'] as $attr){ ?>
                    <li class="popup__card-list-item">
                        <span class="popup__card-list-title"><?= $attr['name']; ?>:</span>
                        <span class="popup__card-list-value"><?= $attr['text']; ?></span>
                    </li>
                <?php } ?>
            <?php } ?>
        ...
        <?php if ($option['type'] == 'checkbox') { ?>
         ...
         <?php if ($option['type'] == 'image') { ?>
         <?php if($cosyone_image_options == 'thumbs'){ ?>
            <li id="input-option<?php echo $option['product_option_id']; ?>" class="popup__card-list-item">
                <span class="popup__card-list-title"><?php echo $option['name']; ?>: </span>
                <div class="popup__card-list-colors form-group <?php echo ($option['required'] ? ' required' : ''); ?>">
                <?php foreach ($option['product_option_value'] as $option_value) { ?>
                    <label class="" for="option-value-<?php echo $option_value['product_option_value_id']; ?>">
                      <input class="mfp-hide" type="radio" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option_value['product_option_value_id']; ?>" id="option-value-<?php echo $option_value['product_option_value_id']; ?>" />
                      <span class="popup__card-list-color">
      ...
      <?php } ?> 
        ...
        <div class="popup__card-actions">
            <button class="button _black js-show-checkout-popup">Купить</button>
            <a href="<?= $similar; ?>" class="button _transparent">Смотреть похожие</a>
        </div>
    </div>


<script>
    $('.js-show-checkout-popup').on('click', function() {
	$.ajax({
		url: 'index.php?route=checkout/cart2/add',
		type: 'post',
		data: $('#product input[type=\'text\'], #product input[type=\'hidden\'], #product input[type=\'radio\']:checked, #product input[type=\'checkbox\']:checked, #product select, #product textarea'),
		dataType: 'json',
		success: function(json) {
			...
			if (json['success']) {
                            ...
                            $('#one_click_form input[name=\'option\']').attr('value', json['option']);
                            //$('#one_click_form input[name=\'name\']').attr('value', json['option']);
                            //console.log(json['success']);
				$.magnificPopup.open({
                                    items: {
                                        src: '.js-checkout-popup',
                                        type: 'inline'
                                    }
                                })
			}
